...
def ask_ports():
    while True:
        raw = input("TCP ports to test (example: 80 or 80,443,22): ").strip()
        if not raw:
            print("Please enter at least one port.")
            continue
        try:
            ports = []
...
def tcp_probe(ip: str, port: int, timeout: float, retries: int):
    last_error = None
...
def make_tasks(batch_ips, ports):
    for ip in batch_ips:
        for port in ports:
            yield (ip, port)
...
class ResultWriter:
    def __init__(self, base_filename: str):
        ...
        self.csv_writer = csv.DictWriter(
            self.csv_file,
            fieldnames=["timestamp", "ip", "port", "status", "latency_ms", "error", "attempts"]
        )
...
def print_batch_summary(batch_results, batch_elapsed, completed_units, total_units):
    ...
    rate = completed_units / batch_elapsed if batch_elapsed > 0 else 0
...
def main():
    ...
    output_base = input("Output filename base (example: tcp_results): ").strip() or "tcp_results"
    ...
    skip_scanned = ask_yes_no("Skip already-scanned IP:PORT entries from previous run?", default=True)
    ...
    total_units = len(ips) * len(ports)
    completed_units = 0

    if skip_scanned:
        completed_units = sum(1 for ip in ips for port in ports if (ip, port) in already_scanned)
    ...
    while offset < len(ips):
        for _ in range(batches_to_run):
            ...
            tasks = [(ip, port) for ip in batch_ips for port in ports if (ip, port) not in already_scanned]
            ...
            if not tasks:
                print("Nothing to do in this batch (all entries already scanned/skipped).")
            ...
            with ThreadPoolExecutor(max_workers=min(worker_count, len(tasks) or 1)) as executor:
                future_map = {
                    executor.submit(tcp_probe, ip, port, timeout, retries): (ip, port)
                ...
                for future in as_completed(future_map):
                    ...
                    already_scanned.add((result["ip"], result["port"]))
                    ...
                    if result["status"] == "success":
                        ...
                        print(f"[OK] {result['ip']}:{result['port']} ({result['latency_ms']} ms)")
